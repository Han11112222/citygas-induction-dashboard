...
if selected_menu == "1. 전환 추세 및 상세 분석":
    ...
    # [단위 변환] 천m³로 통일
    df_year['손실추정_천m3'] = df_year['월별손실추정_m3'] / 1000
    ...
    # ----------------------------------------------------
    # 그래프 2: 가정용 판매량 (상하 배치, Full Width)
    # ----------------------------------------------------
    ...
    df_r['손실추정_천m3'] = df_r['월별손실추정_m3'] / 1000
    
    # [형님 요청] '가정용 전체 판매량' (전사 기준, 천m3)
    if not df_sales_raw.empty:
    ...
    with c6:
        # [형님 요청] 전사 가정용 판매량(천m3) vs 지역 손실량(천m3)
        ...
        fig_r2.update_layout(
            title=f"[{sel_region}] 손실 추정량 vs 전사 가정용 판매량 (단위: 천m³)", 
            legend=dict(orientation="h", y=-0.2),
            yaxis=dict(title="전사 판매량 (천m³)"),
