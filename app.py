...
def load_data_final_v7(url): # v7: 캐시 갱신
    try:
...
@st.cache_data(ttl=60)
def load_sales_data_final_v7(): # v7: 캐시 갱신
    """
    [단위 보정 최종]
    - 원본 데이터: 천m³
...
df_raw = load_data_final_v7(gas_url)
df_sales_raw = load_sales_data_final_v7()
...
with st.sidebar:
    ...
    st.caption("※ PPH 변경 시 손실 추정량(연한 파란색)과 비중(빨간 선)이 변동됩니다.")
...
# =========================================================
    ...
    with col_v2_2:
        ...
        fig_loss.add_trace(go.Bar(
            ...
            name='연간 손실량',
            marker_color=COLOR_INDUCTION,
            text=df_ver2['월별손실추정'].apply(lambda x: f"{x:,.0f}"),
            textposition='auto'
        ))
