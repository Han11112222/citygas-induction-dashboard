...
if selected_menu == "1. 전환 추세 및 상세 분석":
    ...
    with col1:
        ...
        fig_u1.add_trace(go.Scatter(
            x=df_year_filtered['Year'], y=df_year_filtered['손실점유율_가정'],
        ...
        fig_u1.update_layout(barmode='stack', legend=dict(orientation="h", y=1.1), height=400)
    ...
    with col2:
        ...
        fig_u2.add_trace(go.Scatter(
            x=df_year_filtered['Year'], y=df_year_filtered['손실점유율_전체'],
            mode='lines+markers+text',
            text=df_year_filtered['손실점유율_전체'].apply(lambda x: f"{x:.1f}%"),
            textposition="top center",
            name='손실 비중(전체 대비)',
            line=dict(color=COLOR_LINE, width=2)
        ), secondary_y=True)
        
        fig_u2.update_layout(barmode='stack', legend=dict(orientation="h", y=1.1), height=400)
    ...
    with c6:
        # [수정] 상세분석: 전사 전체 판매량(배경) vs 해당 지역 손실 추정량(강조)
