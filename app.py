...
def load_data_final_v6(url): # v6: 캐시 강제 초기화
    try:
...
@st.cache_data(ttl=60)
def load_sales_data_final_v6(): # v6: 캐시 강제 초기화
    """
    [단위 보정 최종]
    - 원본 데이터: 천m³
...
# --- [디자인] 컬러 팔레트 ---
COLOR_GAS = '#1f77b4'       # 진한 파랑 (판매량/누적)
COLOR_INDUCTION = '#a4c2f4' # 연한 하늘색 (손실량/누적)
COLOR_LINE = '#d62728'      # 빨강 (비율/손실선)
...
df_raw = load_data_final_v6(gas_url)
df_sales_raw = load_sales_data_final_v6()
...
with st.sidebar:
    ...
    st.caption("※ PPH 변경 시 그래프의 빨간색 선(손실 추정량)이 움직입니다.")
...
if selected_menu == "1. 전환 추세 및 상세 분석":
    ...
    # ----------------------------------------------------
    # 그래프 1: 세대 구성 (하이라이트 포함) - [형님 지시] 원상복구
    # ----------------------------------------------------
    ...
    fig_q.update_layout(barmode='stack', legend=dict(orientation="h", y=1.1), height=600)
    ...
    with col1:
        ...
        # 2축: 손실량 (빨강 선) - [핵심] 막대 대신 선으로 표현하여 움직임 극대화
        fig_u1.add_trace(go.Scatter(x=df_year_filtered['Year'], y=df_year_filtered['월별손실추정'], name='손실량 (우측축)', marker_color=COLOR_LINE, mode='lines+markers', line=dict(width=4)), secondary_y=True)
        
        fig_u1.update_layout(legend=dict(orientation="h", y=1.1), height=500)
        fig_u1.update_yaxes(title_text="판매량 (m³)", secondary_y=False)
        fig_u1.update_yaxes(title_text="손실 추정량 (m³)", secondary_y=True)
    ...
    with col2:
        ...
        # 2축: 손실량 (빨강 선)
        fig_u2.add_trace(go.Scatter(x=df_year_filtered['Year'], y=df_year_filtered['월별손실추정'], name='손실량 (우측축)', marker_color=COLOR_LINE, mode='lines+markers', line=dict(width=4)), secondary_y=True)
        
        fig_u2.update_layout(legend=dict(orientation="h", y=1.1), height=500)
        fig_u2.update_yaxes(title_text="전체 판매량 (m³)", secondary_y=False)
        fig_u2.update_yaxes(title_text="손실 추정량 (m³)", secondary_y=True)
    ...
    st.caption("※ 그래프는 이중축(Dual Y-Axis)을 사용하여 판매량(좌측)과 손실량(우측)을 동시에 표현했습니다. PPH 변경 시 빨간 선이 움직입니다.")
    ...
    # [4] 상세분석: 지역별 흐름 - [형님 지시] 판매량(회색) 삭제, 손실량만 표시
    ...
    # [형님 요청] 전사 '가정용' 판매량 (m3) - *필요 시 사용 (여기선 그래프에 표시 안함)*
    if not df_sales_raw.empty:
    ...
    with c6:
        # [형님 지시] 회색(판매량) 제거하고 손실량(파란색)만 표시
